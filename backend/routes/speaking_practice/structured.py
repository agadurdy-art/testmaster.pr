...
this module holds the sync endpoint + form parsing + validation + persistence
helpers; the durable job queue lives in jobs.py. Behavior unchanged.
"""
...
async def _emit_telemetry(event: Dict[str, Any]) -> None:
    if _state.db is None:
        return
    try:
        await _state.db.telemetry_events.insert_one(event)
...
async def _persist_structured_attempt(
    *,
    ...
    if _state.db is None:
        return
    try:
        await _state.db.speaking_practice_structured_attempts.insert_one(
            {
                "_id": uuid.uuid4().hex,
                "user_id": user_id,
