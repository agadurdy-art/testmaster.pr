#!/usr/bin/env python3
"""
Update seed_advanced_mastery.py to include listening content
for the first 5 modules directly in the seed file.
"""
...
def update_seed_file():
    ...
    # For each module, add listening content
    for module_num, listening in LISTENING_DATA.items():
        # Find the module and add listening after quiz
        pattern = rf"('id': 'advanced-module-{module_num}'.*?'quiz':.*?\}})\]?\}}\}}"
        
        # Convert listening dict to Python string
        listening_str = repr(listening)
        
        # We need to add 'listening': {...} after the quiz section
        # This is tricky, so let's do it module by module
        
    # Alternative approach: Just update the database directly and also update seed
    print("Listening data prepared. Use sync script to update production.")
    
    # Write to a temporary Python file that can be imported
    output = f'''#!/usr/bin/env python3
"""Listening content for Advanced Mastery modules 1-5"""

LISTENING_CONTENT = {repr(LISTENING_DATA)}
'''
    
    with open('/app/backend/listening_content.py', 'w') as f:
        f.write(output)
    
    print("✅ Created /app/backend/listening_content.py")
