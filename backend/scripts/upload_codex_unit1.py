#!/usr/bin/env python3
"""
Upload Aga's Codex-generated Unit 1 vocab images to R2.

Source folder: ~/Desktop/Stage3_Unit1_All_about_me_Images2/
Filenames: NN-<word>.png (numeric prefix is dropped for the R2 key,
"the uk" → "the_uk", "the usa" → "the_usa").
...
SRC = Path.home() / "Desktop" / "Stage3_Unit1_All_about_me_Images2"
ENRICHED = REPO_ROOT / "backend" / "content" / "enriched"
R2_PUBLIC_BASE = "https://pub-fcd31e7869f94c4896d039253b8f1646.r2.dev"
R2_PREFIX = "vocab"

# Codex filename → enriched-JSON word. Most are identical to the part
# after the numeric prefix, but "uk" and "usa" are stored in the
# wordlist as "the UK" / "the USA".
FILENAME_TO_WORD = {
    "uk": "the UK",
    "usa": "the USA",
}
...
def main():
    ...
    # Step 1: upload every PNG, build word → R2 URL map.
    word_to_url = {}
    files = sorted(SRC.glob("*.png"))
    print(f"Found {len(files)} files in {SRC}")
    for f in files:
        # Strip numeric prefix: "01-bag.png" → "bag"
    ...
    # Step 2: rewrite Stage 3 enriched JSONs. Match on `word` field
    # (case-insensitive), set image_url everywhere it appears (vocab
    # items, game items, distractors, options_full).
    total_refs = 0
