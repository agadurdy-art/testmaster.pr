...
export default function ReadingPracticeMasteryAcademic({ user }) {
  ...
  const selectModule = async (moduleId) => {
    try {
      setLoading(true);
      const res = await fetch(`${API_URL}/api/courses/reading/mastery/academic/${moduleId}`);
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <Badge className="bg-green-600 text-white">MASTERY</Badge>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex gap-2 flex-wrap">
            {filteredModules.map((module) => (
              <Button
                ...
                className={selectedModule === module.module_id ? 'bg-green-600' : ''}
              >
            ...
            <Card className="p-0 overflow-hidden">
              <div className="bg-gradient-to-r from-green-600 to-emerald-600 text-white p-4">
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  ...
                  <Badge className="bg-green-400/30">{questionTypes[moduleContent.question_type]?.name}</Badge>
                </div>
                <h2 className="text-lg font-bold">{moduleContent.title}</h2>
                <p className="text-sm text-green-100 mt-1">{moduleContent.text_type}</p>
              </div>
                ...
                <Card key={idx} className={`p-4 ${submitted ? (results?.questionResults[idx]?.isCorrect ? 'border-green-300 bg-green-50' : 'border-red-300 bg-red-50') : ''}`}>
                    ...
                    {q.skill_tested && (
                      <div className="flex gap-1 flex-wrap">
                        {q.skill_tested.map((skill, si) => (
                          <Badge key={si} className="bg-green-100 text-green-700 text-xs">{skill}</Badge>
                        ))}
                      </div>
                    )}
                  </div>
                  ...
                  {q.options ? (
                    <div className="space-y-2 ml-4">
                      {q.options.map((opt, i) => (
                        <label key={i} className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer hover:text-green-600">
                          <input 
                            type="radio" 
                            name={`q_${idx}`} 
                            value={opt.charAt(0)}
                    ...
                    <div className="flex gap-4 ml-4">
                      {['True', 'False', 'Not Given'].map(opt => (
                        <label key={opt} className="flex items-center gap-2 text-sm cursor-pointer hover:text-green-600">
                          <input 
                            type="radio" 
                            name={`q_${idx}`} 
                            value={opt}
                    ...
                    <input 
                      ...
                      onChange={(e) => handleAnswerChange(idx, e.target.value)}
                      disabled={submitted}
                      className="w-full p-3 border rounded-lg text-sm focus:border-green-500" 
                    />
                  )}
