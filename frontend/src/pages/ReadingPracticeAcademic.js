...
export default function ReadingPracticeAcademic({ user }) {
  ...
  const selectModule = async (moduleId) => {
    try {
      setLoading(true);
      const res = await fetch(`${API_URL}/api/courses/reading/academic/advanced/${moduleId}`);
      const data = await res.json();
      if (data.success) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading Academic Reading...</p>
        </div>
      </div>
    );
  }
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                  <BookOpen className="w-5 h-5 text-blue-600" /> Academic Reading Practice
                </h1>
                <p className="text-sm text-gray-500">Research articles & academic texts</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              {/* Timer */}
              ...
              <Badge className="bg-blue-600 text-white">ACADEMIC</Badge>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-4">
              <Card className="p-4 bg-blue-50 border-blue-200">
                ...
                <p className="text-sm text-blue-600 mt-1">
                  Answer all {moduleContent.reading_scenario?.questions?.length || 0} questions based on the passage.
                </p>
              </Card>
                ...
                <Card key={idx} className={`p-4 ${submitted ? (results?.questionResults[idx]?.isCorrect ? 'border-green-300 bg-green-50' : 'border-red-300 bg-red-50') : ''}`}>
                    ...
                    {q.skill_tested && (
                      <div className="flex gap-1 flex-wrap">
                        {q.skill_tested.map((skill, si) => (
                          <Badge key={si} className="bg-blue-100 text-blue-700 text-xs">{skill}</Badge>
                        ))}
                      </div>
                    )}
                  </div>
                  ...
                  {q.options ? (
                    <div className="space-y-2 ml-4">
                      {q.options.map((opt, i) => (
                        <label key={i} className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer hover:text-blue-600">
                          <input 
                            type="radio" 
                            name={`q_${idx}`} 
                            value={opt}
                    ...
                    <div className="flex gap-4 ml-4">
                      {['True', 'False', 'Not Given'].map(opt => (
                        <label key={opt} className="flex items-center gap-2 text-sm cursor-pointer hover:text-blue-600">
                          <input 
                            type="radio" 
                            name={`q_${idx}`} 
                            value={opt}
                    ...
                    <input 
                      ...
                      onChange={(e) => handleAnswerChange(idx, e.target.value)}
                      disabled={submitted}
                      className="w-full p-3 border border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500" 
                    />
                  )}
              ...
              {!submitted ? (
                <Button 
                  ...
                  className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-6"
                ...
                <Card className="p-6 bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200">
                  <h3 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                    <Award className="w-6 h-6 text-blue-600" /> Your Results
                  </h3>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button onClick={() => navigate('/question-bank')} className="flex-1 bg-blue-600">
