...
// in WritingPractice.js / FullTestResults.js so Task2 results show a Liz card.
function buildLizMessageT2(result) {
...
export default function WritingTask2Practice() {
  ...
  // Stable idempotency key — keeps a single Sonnet bill across retries.
  ...
  if (evaluation) {
    return (
      <WritingEvaluatorResult
        ...
        title="Writing Task 2 — Result"
        lizMessage={buildLizMessageT2(evaluation)}
        onBack={goBack}
        onRewrite={() => {
          setEvaluation(null);
