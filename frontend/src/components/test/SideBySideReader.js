...
const SideBySideReader = ({
  ...
  return (
    <div className={`bg-white rounded-lg shadow-lg border ${className}`}>
          ...
          <div className="flex items-center gap-1 mr-2 border-r pr-2">
            <button
              ...
              className={`p-1.5 rounded transition-colors ${
                highlightMode ? 'bg-yellow-400 text-white' : 'hover:bg-gray-200 text-gray-500'
              }`}
              title={highlightMode ? 'Highlighter ON - Select text to highlight' : 'Turn on highlighter'}
            >
              ...
              <>
                {highlightColors.map((color) => (
                  <button
                    ...
                    className={`w-5 h-5 rounded ${color.bg} ${color.hover} border-2 ${
                      highlightColor === color.name ? 'border-gray-700' : 'border-transparent'
                    }`}
      ...
      {highlightMode && (
        <div className="bg-yellow-100 px-4 py-2 text-sm text-yellow-800 flex items-center justify-between">
          <span>✏️ Highlighter Mode: Select text to highlight. Click highlighted text to remove.</span>
          <span className="text-xs text-yellow-600">{highlights.length} highlight(s)</span>
        </div>
      )}
      ...
      >
        ...
        >
          ...
          <div className="text-gray-700 leading-relaxed text-base whitespace-pre-wrap select-text">
